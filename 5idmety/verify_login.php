...
// التحقق من وجود جلسة مؤقتة لتسجيل الدخول
if (!isset($_SESSION['temp_user_id']) || !isset($_SESSION['login_step']) || $_SESSION['login_step'] !== 'login_verification') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        if (empty($otp_code)) {
            ...
            // التحقق من الرمز
            if (verifyOTP($db, $user_id, 'login_verification', $otp_code)) {
                // تسجيل دخول ناجح
                ...
                // تسجيل النشاط
                logActivity($db, $user_data['id'], 'login_success', 'تسجيل دخول ناجح مع التحقق من الرمز');
                ...
                // رسالة ترحيب
                $_SESSION['welcome_message'] = "مرحباً بك مرة أخرى، " . $user_data['fullname'];
                
                redirectByRole();
            } else {
                $error = "رمز التحقق غير صحيح أو منتهي الصلاحية";
                logActivity($db, $user_id, 'login_failed_otp', 'فشل في التحقق من رمز تسجيل الدخول');
            }
        }
    }
}
...
if (isset($_GET['resend'])) {
    try {
        ...
        createVerification($db, $user_id, 'login_verification', $otp_code);
        
        $email_message = "مرحباً {$user_data['fullname']}،\n\nرمز التحقق الجديد لتسجيل الدخول هو: $otp_code\n\nهذا الرمز صالح لمدة " . OTP_EXPIRY_MINUTES . " دقائق فقط.\n\nإذا لم تكن أنت من يحاول تسجيل الدخول، يرجى تجاهل هذه الرسالة.\n\nشكراً لك،\nفريق منصة موريتانيا";
        sendEmail($user_email, "رمز التحقق الجديد لتسجيل الدخول", $email_message);
        
        logActivity($db, $user_id, 'login_otp_resent', 'تم إعادة إرسال رمز التحقق لتسجيل الدخول');
...
<head>
    ...
    <title>التحقق من تسجيل الدخول - منصة موريتانيا</title>
    ...
    <div class="max-w-md w-full space-y-8">
        ...
        <div class="text-center">
            ...
            <p class="mt-2 text-sm text-gray-200">
                مرحباً <span class="text-yellow-300 font-medium"><?= htmlspecialchars($user_data['fullname']) ?></span>
            </p>
        ...
        <div class="bg-white rounded-lg shadow-xl p-8">
            ...
            <div class="mt-6 space-y-3 text-center">
                <div>
                    ...
                    <a href="verify_login.php?resend=1" class="text-sm font-medium text-green-600 hover:text-green-500 transition">
                        إعادة الإرسال
                    </a>
                </div>
                ...
                <div class="flex">
                    ...
                    <div class="text-sm text-blue-700">
                        ...
                        <ul class="text-xs space-y-1">
                            ...
                            <li>• هذا الإجراء يحمي حسابك من الوصول غير المصرح به</li>
                        </ul>
                    </div>
                </div>
            </div>
